  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Smoothly move from firmware home to zero positions (straight up) over 2 seconds
  // using firmware's quintic trajectory (T parameter). This prevents a violent snap.
  // After this, the arm matches hw_commands_ = [0.0, 0.0, 0.0, 0.0].
  ...
  zero_cmd << " " << J5_FIXED_US << " " << J6_FIXED_US << " T 2000\n";

  RCLCPP_INFO(rclcpp::get_logger("ServoHardwareInterface"),
    "Moving to zero position (2s trajectory): %s", zero_cmd.str().c_str());
  send_command(zero_cmd.str());
  usleep(3000000);  // Wait for 2s trajectory + 1s buffer
